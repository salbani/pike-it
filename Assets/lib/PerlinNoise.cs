using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class PerlinNoise
{
  public static System.Random RNGSingleton = new System.Random();
  static int[] permutation;
  static Vector3[] directions;
  static int permutationCount;
  static int directionsCount;

  static PerlinNoise()
  {
    GeneratePermutation(123);
  }

  static public void GenerateOriginialPerlinPermutation()
  {
    permutationCount = 255;
    permutation = new int[]{
        151,160,137, 91, 90, 15,131, 13,201, 95, 96, 53,194,233,  7,225,
        140, 36,103, 30, 69,142,  8, 99, 37,240, 21, 10, 23,190,  6,148,
        247,120,234, 75,  0, 26,197, 62, 94,252,219,203,117, 35, 11, 32,
         57,177, 33, 88,237,149, 56, 87,174, 20,125,136,171,168, 68,175,
         74,165, 71,134,139, 48, 27,166, 77,146,158,231, 83,111,229,122,
         60,211,133,230,220,105, 92, 41, 55, 46,245, 40,244,102,143, 54,
         65, 25, 63,161,  1,216, 80, 73,209, 76,132,187,208, 89, 18,169,
        200,196,135,130,116,188,159, 86,164,100,109,198,173,186,  3, 64,
         52,217,226,250,124,123,  5,202, 38,147,118,126,255, 82, 85,212,
        207,206, 59,227, 47, 16, 58, 17,182,189, 28, 42,223,183,170,213,
        119,248,152,  2, 44,154,163, 70,221,153,101,155,167, 43,172,  9,
        129, 22, 39,253, 19, 98,108,110, 79,113,224,232,178,185,112,104,
        218,246, 97,228,251, 34,242,193,238,210,144, 12,191,179,162,241,
         81, 51,145,235,249, 14,239,107, 49,192,214, 31,181,199,106,157,
        184, 84,204,176,115,121, 50, 45,127,  4,150,254,138,236,205, 93,
        222,114, 67, 29, 24, 72,243,141,128,195, 78, 66,215, 61,156,180,

        151,160,137, 91, 90, 15,131, 13,201, 95, 96, 53,194,233,  7,225,
        140, 36,103, 30, 69,142,  8, 99, 37,240, 21, 10, 23,190,  6,148,
        247,120,234, 75,  0, 26,197, 62, 94,252,219,203,117, 35, 11, 32,
         57,177, 33, 88,237,149, 56, 87,174, 20,125,136,171,168, 68,175,
         74,165, 71,134,139, 48, 27,166, 77,146,158,231, 83,111,229,122,
         60,211,133,230,220,105, 92, 41, 55, 46,245, 40,244,102,143, 54,
         65, 25, 63,161,  1,216, 80, 73,209, 76,132,187,208, 89, 18,169,
        200,196,135,130,116,188,159, 86,164,100,109,198,173,186,  3, 64,
         52,217,226,250,124,123,  5,202, 38,147,118,126,255, 82, 85,212,
        207,206, 59,227, 47, 16, 58, 17,182,189, 28, 42,223,183,170,213,
        119,248,152,  2, 44,154,163, 70,221,153,101,155,167, 43,172,  9,
        129, 22, 39,253, 19, 98,108,110, 79,113,224,232,178,185,112,104,
        218,246, 97,228,251, 34,242,193,238,210,144, 12,191,179,162,241,
         81, 51,145,235,249, 14,239,107, 49,192,214, 31,181,199,106,157,
        184, 84,204,176,115,121, 50, 45,127,  4,150,254,138,236,205, 93,
        222,114, 67, 29, 24, 72,243,141,128,195, 78, 66,215, 61,156,180
    };

    directionsCount = 16;
    directions = new Vector3[]{
        new Vector3( 1f, 1f, 0f),
        new Vector3(-1f, 1f, 0f),
        new Vector3( 1f,-1f, 0f),
        new Vector3(-1f,-1f, 0f),
        new Vector3( 1f, 0f, 1f),
        new Vector3(-1f, 0f, 1f),
        new Vector3( 1f, 0f,-1f),
        new Vector3(-1f, 0f,-1f),
        new Vector3( 0f, 1f, 1f),
        new Vector3( 0f,-1f, 1f),
        new Vector3( 0f, 1f,-1f),
        new Vector3( 0f,-1f,-1f),

        new Vector3( 1f, 1f, 0f),
        new Vector3(-1f, 1f, 0f),
        new Vector3( 0f,-1f, 1f),
        new Vector3( 0f,-1f,-1f)
    };
  }

  static public void GeneratePermutation(int seed, int binaryFactor = 8)
  {
    permutationCount = (int)Mathf.Pow(2, Mathf.Abs(binaryFactor));
    List<int> permutationNumbers = new List<int>();

    permutation = new int[permutationCount * 2];
    Random.InitState(seed);

    for (int i = 0; i < permutationCount; i++)
    {
      permutationNumbers.Add(i);
    }
    for (int i = 0; i < permutationCount; i++)
    {
      int randomIndice = Random.Range(0, permutationNumbers.Count);
      permutation[i] = permutationNumbers[randomIndice];
      permutation[permutationCount + i] = permutationNumbers[randomIndice];
      permutationNumbers.RemoveAt(randomIndice);
    }

    List<Vector3> directionVectors = new List<Vector3>();
    for (int x = -1; x <= 1; x++)
    {
      for (int y = -1; y <= 1; y++)
      {
        for (int z = -1; z <= 1; z++)
        {
          if (x == 0 && y == 0 && z == 0)
            continue;
          directionVectors.Add(new Vector3(x, y, z).normalized);
        }
      }
    }

    directionsCount = directionVectors.Count - 1;
    directions = new Vector3[directionsCount];
    for (int i = 0; i < directionsCount; i++)
    {
      int randomIndice = Random.Range(0, permutationNumbers.Count);
      directions[i] = directionVectors[randomIndice];
      directionVectors.RemoveAt(randomIndice);
    }
  }
  private static float scalar(Vector3 a, Vector3 b)
  {
    return a.x * b.x + a.y * b.y + a.z * b.z;
  }

  private static float smoothDistance(float d)
  {
    return d * d * d * (d * (d * 6f - 15f) + 10f);
  }

  static public float Get3D(Vector3 point)
  {
    return Get3D(point.x, point.y, point.z);
  }

  static public float Get3D(float x, float y, float z)
  {

    int flooredPointX0 = Mathf.FloorToInt(x);
    int flooredPointY0 = Mathf.FloorToInt(y);
    int flooredPointZ0 = Mathf.FloorToInt(z);

    float distanceX0 = x - flooredPointX0;
    float distanceY0 = y - flooredPointY0;
    float distanceZ0 = z - flooredPointZ0;

    float distanceX1 = distanceX0 - 1f;
    float distanceY1 = distanceY0 - 1f;
    float distanceZ1 = distanceZ0 - 1f;

    flooredPointX0 &= permutationCount - 1;
    flooredPointY0 &= permutationCount - 1;
    flooredPointZ0 &= permutationCount - 1;

    int flooredPointX1 = flooredPointX0 + 1;
    int flooredPointY1 = flooredPointY0 + 1;
    int flooredPointZ1 = flooredPointZ0 + 1;

    int permutationX0 = permutation[flooredPointX0];
    int permutationX1 = permutation[flooredPointX1];

    int permutationY00 = permutation[permutationX0 + flooredPointY0];
    int permutationY10 = permutation[permutationX1 + flooredPointY0];
    int permutationY01 = permutation[permutationX0 + flooredPointY1];
    int permutationY11 = permutation[permutationX1 + flooredPointY1];

    Vector3 direction000 = directions[permutation[permutationY00 + flooredPointZ0] % directionsCount];
    Vector3 direction100 = directions[permutation[permutationY10 + flooredPointZ0] % directionsCount];
    Vector3 direction010 = directions[permutation[permutationY01 + flooredPointZ0] % directionsCount];
    Vector3 direction110 = directions[permutation[permutationY11 + flooredPointZ0] % directionsCount];
    Vector3 direction001 = directions[permutation[permutationY00 + flooredPointZ1] % directionsCount];
    Vector3 direction101 = directions[permutation[permutationY10 + flooredPointZ1] % directionsCount];
    Vector3 direction011 = directions[permutation[permutationY01 + flooredPointZ1] % directionsCount];
    Vector3 direction111 = directions[permutation[permutationY11 + flooredPointZ1] % directionsCount];

    float value000 = scalar(direction000, new Vector3(distanceX0, distanceY0, distanceZ0));
    float value100 = scalar(direction100, new Vector3(distanceX1, distanceY0, distanceZ0));
    float value010 = scalar(direction010, new Vector3(distanceX0, distanceY1, distanceZ0));
    float value110 = scalar(direction110, new Vector3(distanceX1, distanceY1, distanceZ0));
    float value001 = scalar(direction001, new Vector3(distanceX0, distanceY0, distanceZ1));
    float value101 = scalar(direction101, new Vector3(distanceX1, distanceY0, distanceZ1));
    float value011 = scalar(direction011, new Vector3(distanceX0, distanceY1, distanceZ1));
    float value111 = scalar(direction111, new Vector3(distanceX1, distanceY1, distanceZ1));

    float smoothDistanceX = smoothDistance(distanceX0);
    float smoothDistanceY = smoothDistance(distanceY0);
    float smoothDistanceZ = smoothDistance(distanceZ0);

    return Mathf.Lerp(
        Mathf.Lerp(Mathf.Lerp(value000, value100, smoothDistanceX), Mathf.Lerp(value010, value110, smoothDistanceX), smoothDistanceY),
        Mathf.Lerp(Mathf.Lerp(value001, value101, smoothDistanceX), Mathf.Lerp(value011, value111, smoothDistanceX), smoothDistanceY),
        smoothDistanceZ);
  }
}